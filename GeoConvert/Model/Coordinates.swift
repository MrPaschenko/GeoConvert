...
let π = 3.14159265358979 // Value of Pi
...
let e2P = 2 * alP - pow(alP, 2) // Eccentricity squared
...
let e2W = 2 * alW - pow(alW, 2) // Eccentricity squared

// Auxiliary values for ellipsoid transformation
let a: Double = Double((aP + aW)) / 2.0
let e2 = (e2P + e2W) / 2
let da: Double = Double(aW - aP)
let de2 = e2W - e2P
...
func dL(Bd: Double, Ld: Double, H: Double) -> Double {
    let B = degreesToRadians(Bd)
    let L = degreesToRadians(Ld)
    let N = a * pow((1 - e2 * pow(sin(B), 2)), -0.5)
...
func WGS84Alt(Bd: Double, Ld: Double, H: Double) -> Double {
    let B = degreesToRadians(Bd)
    let L = degreesToRadians(Ld)
    let N = a * pow((1 - e2 * pow(sin(B), 2)), -0.5)
...
func SK42_WGS84(latitude: Double, longtitude: Double, height: Double) -> [Double] {
    ...
    return [newLatitude, newLongtitude, 0]
}

// Convert WGS84 coordinates to SK42
func WGS84_SK42(latitude: Double, longtitude: Double, height: Double) -> [Double] {
